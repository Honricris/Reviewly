...
interface ReportGeneratorProps {
  onClose: () => void;
  initialReport?: { type: string; parameters: any } | null;
}

const ReportGenerator: React.FC<ReportGeneratorProps> = ({ onClose, initialReport }) => {
  const [reportType, setReportType] = useState<string>(initialReport?.type || '');
  const [formData, setFormData] = useState<any>(initialReport?.parameters || {});
...
export const generateProductPopularityReport = async (
  doc: jsPDF,
  pageNumber: number,
  parameters: any,
  addHeaderFooter: (doc: jsPDF, title: string, pageNumber: number) => void
...
export const generateReportPDF = async (reportType: string, parameters: any) => {
  const doc = new jsPDF();
  let pageNumber = 1;
  ...
  try {
    switch (reportType) {
      case 'user_activity':
        await generateUserActivityReport(doc, pageNumber, parameters, addHeaderFooter);
        break;
      case 'product_popularity':
        await generateProductPopularityReport(doc, pageNumber, parameters, addHeaderFooter);
        break;
      default:
        throw new Error('Invalid report type');
    }
